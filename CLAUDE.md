...
> **Milestone: M3 — Overworld. Steps S1–S4 done ✅ (2026-07-09/10): Phaser's in, The Meadows renders, and you walk it with an animated hero (trees block you). ROADMAP M3 rows 1–2 done → 16/31 steps. M2 is FULLY complete ✅ (save v1 + export/import). The overworld is now the hub — a battle is a detour reached via the temporary "Battle test" button until the S7 handoff. Next: S5/S6 (wild Fakeamon standing on the map) → S7 (bump → real battle → back).** *(Update this line as we progress.)*
