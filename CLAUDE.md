...
> **Milestone: M2 — Catching & Team. In progress: Steps 1–5 done ✅, plus M5-plan S1 ✅** *(Update this line as we progress.)*
> M1 (the battle slice) is complete and live. M2 so far: code split into `src/` files (Step 1), Leafick + a Choose Your Starter screen (Step 2), a random wild opponent every battle via the `startBattle(config)` contract with a Run button that always works (Step 3), a **Throw Fakeaball** catch action using the 50%-base capture formula, with a Gotcha!/broke-free log and a counter-attack if the catch fails (Step 4), the M5-plan **S1** state foundation: `FAKEAMON` species are now keyed (`growler`/`whaley`/`leafick`) with `base*` stats, `src/state.js` adds `newIndividual()`/`statsFor()`, and every fighter in `battle.js`/`main.js` is an individual with its own `currentHP` — the old name-keyed `hp` map is gone, so two of the same species can never share an HP slot again — and now **Step 5** (2026-07-07): a **team list** lives in `gameState.party`/`gameState.box` (`src/state.js`), your chosen starter becomes `party[0]` — the one that fights — and a caught Fakeamon joins an open slot or overflows to the Boxes once the team's full (Lewis's B3: no nicknames, species names only). A new team row + a **Boxes** panel show under the log. **Working default to flag:** `party[0]`'s HP now persists between wild encounters instead of resetting each fight — if it faints, the fight loop pauses with a "needs to rest" message instead of auto-continuing, since there's no way to heal it yet; **Step 6 (Switch)** is how you'd recover. Easy to change if that's not the feel we want. **Next up:** Step 6 — Switch which Fakeamon is fighting, including bringing one in from the Boxes. Lewis cleared the whole homework backlog (B1–B32) on 2026-07-06 — see `DECISIONS.md` rows 14–45. Only B33 (where to swap boxed Fakeamon) is still open, not needed until M5. Live at [jhester599.github.io/fakeamon_spark](https://jhester599.github.io/fakeamon_spark/), auto-deployed via GitHub Pages.
>
