...
**Build now (M4 — world systems):** M1–M3 are complete; **M4S1–S2 are done** (Tokens + the S0 groundwork; the Fakeatent, placeholder art). **Next is M4S3 (Tall Tower)** — a shop building where tokens buy Fakeaballs — then M4S4 (Gym 1 + the one enemy-party engine change), M4S5 (Cooking Cabin), M4S6 (the area-travel seam + The Lagoon). Build against the settled seams: `startBattle(config)` (`src/battle.js`), the screen manager (`src/screens.js`), the save bag (`src/state.js`/`src/save.js`), and the new `WorldScene.rebuildFromState()` resync seam (`src/world/config.js`). **Read `PLANS/M4_WORLD_SYSTEMS_PLAN.md` §A every session, and §2 (state & save) before adding ANY new persistent field** — the shallow-merge trap (CR-B) is the cheapest M4 mistake.

**Do NOT build yet:** evolutions, mini-bosses, Artemis, the box-swap UI (M5-plan S8), Great/Ultra/Cosmic balls, day/night, gym leaders 2–3, and the other four areas' rosters (M4 opens **only** The Lagoon, at M4S6). Within M4, **don't front-run the step order** — the systems depend on each other (tokens before shops/heals; the enemy-party change before Gym 1; the resync seam before multi-map). If a change starts pulling in an M5 system, pause and flag it. *(M1–M3 are the historical record — the battle slice, then catching + a team of 4 with Boxes + switching + save, then the Phaser overworld; full detail in `ROADMAP.md`.)*
