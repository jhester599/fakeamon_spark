...
**Build now (M4 — world systems):** M1–M3 are complete; **M4S1–S3 are done** (Tokens + the S0 groundwork; the Fakeatent with real art; the Tall Tower shop, placeholder art, selling only the basic Fakeaball). **Next is M4S4 (Gym 1)** — the one real engine change (an enemy party/queue for a 2-mon trainer team) plus at least one metal move — then M4S5 (Cooking Cabin), M4S6 (the area-travel seam + The Lagoon). Build against the settled seams: `startBattle(config)` (`src/battle.js`), the screen manager (`src/screens.js`), the save bag (`src/state.js`/`src/save.js`), the building bump seam (`enterBuilding` in `src/main.js` + `spawnBuildings`/`buildingAt`/`handleBuilding` in `src/world/config.js` — adding a new building kind is just a map-data entry + a `BUILDING_LOOKS` fallback + one dispatcher branch), and the `WorldScene.rebuildFromState()` resync seam. **Read `PLANS/M4_WORLD_SYSTEMS_PLAN.md` §A every session, and §2 (state & save) before adding ANY new persistent field** — the shallow-merge trap (CR-B) is the cheapest M4 mistake.
...
/assets/sprites/buildings → map building sprites (M4S2): fakeatent.png (real art, Jeff's own AI-generated tent)
