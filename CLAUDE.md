...
src/main.js         → the conductor: title screen (Continue/New Game + Export/Import), boots the overworld (startWorld), runs starter-select, and owns the battle handoff — enterBattle / startMapEncounter / handleBattleOutcome (bump → battle → back to the map), autosaving after every change; also drives the temporary "Battle test" button
