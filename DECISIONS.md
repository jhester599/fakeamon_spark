...
| 2026-07-25 | 62 | **You can't Run from a gym battle** — the M4 plan (§4.2) flagged this for the M4S4 huddle, since Lewis's B1 ("Run always works") was decided about *wild* battles. Gyms pass `canFlee: false`; wild battles are untouched | (option A) — "once you challenge the leader, you're committed" |
| 2026-07-25 | 63 | **A gym is a BUILDING you bump**, not a trainer standing in the grass — a third entry in The Meadows' town row, reusing the exact `enterBuilding` seam the Fakeatent and Tall Tower already use | (option A) — cheapest path, and no trainer sprite to license |
...
| 2026-08-10 | 76 | **You travel between areas by BOAT, at the east end of The Meadows' path.** Bump it (like a building) rather than walk onto it; it wears a 🔒 until you've won the Gear Badge and a 🚤 after | Jeff & Lewis's M4S6 call. They were offered a gate at the path's end, a boat hidden in the south grass, or no marker at all, and picked the boat *at the path's end* — "given that the lagoon has some water theming, the boat seems like a logical way to travel" |
| 2026-08-10 | 77 | **The Lagoon gets its own Fakeatent and its own berry patches** — but no shop, gym or cabin yet | Jeff & Lewis's M4S6 call: without a tent out there, fainting in The Lagoon would ship you all the way back to The Meadows. A shop was offered and declined for now — one more map-data line whenever they want it |
| 2026-08-10 | 78 | **`defeatedEncounters` stays ONE flat list for all areas, not one list per area** — so no save-version bump for M4S6 | A technical call, logged because `PLANS/M4_WORLD_SYSTEMS_PLAN.md` §5.3 explicitly left it open ("decide at build time; either is JSON-safe"). Encounter ids were already namespaced (`meadows-aardorn`, `lagoon-kroki`), so the flat list already works — and it's the version that needs no migration. Respawning is scoped to the current map instead (`clearedOnThisMap` in `src/main.js`) |
| 2026-08-10 | 79 | **A badge-locked area is gated on `flags.unlockedAreas` alone** — an exit does NOT carry its own `requiresBadge` field | `PLANS/M4_WORLD_SYSTEMS_PLAN.md` §5.3 sketched `requiresBadge` on the exit, but `flags.unlockedAreas` was already pre-declared in save v2 and already written to by `awardGymPrize`. Two places recording the same fact is how they drift apart; the one list is the gate, and the "which badge?" wording on a locked boat is looked up from `GYMS` |
| 2026-08-10 | 80 | **Three design tweaks to M4S6, after seeing it on screen.** (a) **The Lagoon is dark — dark blue and black**, not a cheerful daytime swamp. (b) **The Meadows gets a small inlet of water beside the boat**, so the dock reads as a dock instead of a padlock sitting on grass. (c) **The Lagoon's boat moves to the map's far WEST edge**, mirroring The Meadows' dock on its far east | Jeff & Lewis, straight off the first screenshots. (a) is a `MOOD` block in `tools/make-lagoon-tileset.mjs` — the same George tiles, recoloured, so it stays a dial rather than hand-painted art; the rule is "the bluer a pixel already is, the more light it keeps", which sinks the banks to near-black and leaves the water glowing. (b) needed nine new water tiles in `meadow.png`, added by `tools/add-meadow-water-tiles.mjs` (it grows the image by one row so no existing tile number moves). (c) is continuity: you sail east off one map and arrive at the west edge of the next |
| 2026-08-13 | 82 | **Levels finally change stats: +3 max HP, +2 Attack, +1 Defense, +1 Speed per level** (`STAT_GROWTH_PER_LEVEL`, `src/state.js`) — the four numbers that were all zeros from M2 until M5 Step 1 | Jeff's tuning list, settled with a simulation rather than a guess: four candidate tables were run over 400 fights each. Attack has to climb *faster* than Defense or fights slowly turn into two tanks bouncing off each other; +2 vs +1 keeps a fight about 3 hits long at level 2 **and** at level 15, so the game feels the same all the way through |
| 2026-08-13 | 83 | **Your starter now begins at level 5, not level 1** (`STARTING_LEVEL`) | The moment levels changed stats, a level-1 starter was losing its opening fights in a level 2–5 meadow (simulated: 51% against the weakest wild Fakeamon on the map, 0% against the strongest). Level 5 puts you even with the toughest thing in the starting area — the same trick the classic games use |
| 2026-08-13 | 84 | **The Lagoon's wild Fakeamon drop from level 10–15 to 8–12** | Jeff & Lewis's call once the simulation showed what levelling did to that area: you arrive there at about level 8–10 straight after Gym 1, and being two levels down is very nearly an automatic loss (85% win at level-for-level, 8% one level down, 1% two levels down). At 10–15, most of the area Gym 1's badge had just unlocked was a locked door. They were offered "leave it as a come-back-later area" and "split it into shallows and deeps", and picked the straight re-level. The levels still climb as you go deeper round the water |
| 2026-08-13 | 85 | **The three starters evolve into creatures we already own the art for: Growler → Deviraptor, Whaley → Leviadile, Leafick → Dragarbor** (all three happen to be dragons) | Lewis's call, from a real problem: in the original Tuxemon art **Whaley and Leafick have no evolution at all** — they're already the last form of their families — and Growler's real evolution (Cobarett) isn't in our licensed set. The options were "go and licence their actual relatives" or "pick from the 198 creatures we already have permission to use"; he picked the second, so the step needed no new licence checks. Each pick is a one-word `evolvesTo` edit in `src/data/fakeamon.js` if he changes his mind |
| 2026-08-13 | 86 | **Everything in the game evolves exactly once. Starters evolve at level 16; wild Fakeamon use Tuxemon's own evolution levels, compressed to our shorter game** (15→12, 18→14, 20→16, 24→18, 26→20, 32→22, 41→24) | Jeff's number-tuning slot, filled from real data rather than invented: Tuxemon's own `at_level` values were read from the pinned commit, then squeezed — their game runs to about level 100, ours caps at 30, so uncompressed numbers like 41 would have been unreachable. Compressing while keeping their *ordering* means late bloomers stay late. Second evolutions (some families have a third form) are deliberately left out of this step |
| 2026-08-13 | 87 | **Five new "big" moves exist, because evolving had nothing to upgrade INTO** — Firestorm, Tidal Wave, Vine Lash, Forest Fury, Slam | Discovered while building: the rule is "an evolved Fakeamon uses its new form's moves", but grass had exactly ONE attack in the entire game (Leafage), so a fully evolved grass Fakeamon fought exactly like a baby one. Each new move sits above its type's old best and trades accuracy for power. ⚠️ **The five NAMES are placeholders** — naming moves is Lewis's job, and renaming them is a five-word edit in `src/data/moves.js` |
| 2026-08-11 | 81 | **A building blocks — and opens — every tile along the BOTTOM of its picture, not just the one tile it's pinned to. This AMENDS #71 (it does not cancel it).** The tall parts of a building are still walk-behind | Playtest bug: you could walk straight through the Cooking Cabin's left and right thirds — through its walls — with nothing happening. #71 kept walk-behind because Jeff & Lewis liked strolling behind the Tall Tower and the Gym, which are *tall*; the Cabin is *squat*, so its "behind" is really its middle. Making only the base row solid fixes the Cabin while leaving the tower/gym walk-behind exactly as #71 wanted. The footprint is measured off the sprite's own width (`WorldScene.buildingFootprint`), so there's no number to keep in step with the art |
...
| 2026-07-25 | **M4S4 built — the enemy-party engine change, Gym 1, and the first metal move.** `src/battle.js` gained the M4 plan's §4.1 seam: `config.enemyParty` (an array) + `enemyIndex`, with `activeOpponent()` sitting beside the existing `activePlayer()` so both sides read identically; a lone `config.enemy` is normalised to a bench of one, so **wild battles are behaviourally unchanged** (verified). Faint handling funnels through one new `resolveFaint()` — the single spot that branches "send out the next one" vs "end the battle." **XP moved to per-knockout** (a running `xpEarnedThisBattle` total, reported at the end) so a 2-mon gym pays twice; previously `endBattle` granted it once. Gym content lives in a new `src/data/gyms.js`; the Gym is a **building** on The Meadows' town row (tile 15,7) reusing the `enterBuilding` seam untouched. Win → `GYM_REWARD` + the Gear Badge (a HUD chip) + `theLagoon` pushed onto `unlockedAreas`, so M4S6's gate already has something to open; rematch → `GYM_REMATCH_REWARD` (B17). No save-schema change was needed — `flags.badges/gymsCleared/unlockedAreas` were pre-declared in save v2 (the plan's §2 discipline paying off). **Art:** Allagon vendored + sliced + credited (Spalding004, Chickenshowman — CC BY-SA 4.0, confirmed in `ATTRIBUTIONS.md`). **AV8R deliberately ships with no sprite** — it has no monster entry in `ATTRIBUTIONS.md` (the only "AV8R" there is the *human trainer* "Aviator"), and `wiki.tuxemon.org` is network-blocked here, so per "no asset ships without attribution" it uses the existing colored-box fallback; `CREDITS.md` carries the exact laptop recipe to finish it. Verified with a 34-check headless-browser smoke test plus a separate loss-path test. | The plan called Gym 1 "the only thing in M4 that changes `battle.js`" and asked for the change to be minimal and readable — mirroring the player's own bench (rather than inventing a queue abstraction) keeps it to one new concept a 10-year-old already understands: "the enemy has a bench too" |
| 2026-07-25 | **Gym 1's stats are hand-authored, and its `level` numbers are flavour only.** `STAT_GROWTH_PER_LEVEL` (`src/state.js`) is still all zeros until M5 adds leveling, so a gym mon's `level: 8` / `level: 10` changes **no stats whatsoever** — the M4 plan's §4.3 example stat block would have quietly produced two ordinary-strength monsters. Difficulty therefore comes entirely from base stats: **Allagon** 46/13/14/9 and **AV8R** 52/15/13/14 (vs The Meadows' wild range of HP 36-41, Attack 11-14, Defense 9-13), with AV8R faster than every starter so the ace strikes first. Both carry a loud `[TUNE]` warning not to "buff" a gym by raising its level. Checked against the plan's §4.4 damage-floor invariant: the weakest attacker/move in the game (Leafick + Tackle) still lands 5-6 raw damage, comfortably above the "≥ 3" rule. **Iron Beam's power (14) was deliberately set below Flare (18) and Breech (16)** because metal already gets a 2× bonus against grass — at 18 it would two-shot a Leafick. | Caught during the M4S4 build: the plan's stat block was written before anyone re-checked that leveling does nothing yet. Worth logging loudly, because the same trap is waiting for every gym, mini-boss, and Artemis stat block written before M5 |
