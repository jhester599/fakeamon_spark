...
| 2026-07-08 | **OPEN — Phaser delivery (decide at M3 S1):** load Phaser from a pinned CDN URL **with** a Subresource Integrity hash, **or** vendor `phaser.min.js` into `/assets`. CDN+SRI keeps the repo small; vendoring removes CDN trust entirely and keeps double-click-`index.html` working offline (~1 MB cost). Either is safe — a bare pinned CDN URL without SRI is the only option ruled out (F6) | Surfaced by peer-review checkpoint #1; not worth committing before M3 actually starts, but must be settled at S1 when Phaser is first added |
