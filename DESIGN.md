...
**DECIDED (2026-07-11) — telling apart a mirror match:** the **wild opponent is written as "the wild `<name>`"** everywhere it appears — the battle log, the result messages, the fighter card heading, and the battle title. "The wild Whaley used Splash!", "The wild Whaley fainted!". Because there are no nicknames (B3), this is what keeps a same-species fight (your Whaley vs. a wild Whaley) readable. *(Lewis's call, B34.)*

**BUILT (2026-07-26), with one extension:** a single `fighterName(role)` helper in `src/battle.js` decides what to call each side, so there is one place to change it. Your own Fakeamon keeps its plain name. B34 was decided **before gyms existed**, and "the wild Allagon" would be wrong for a gym leader's Fakeamon — so a trainer battle reads **"Enforcer Boss's Allagon"** instead.
...
**Confirmed by Jeff (2026-07-06):** tech stack — plain HTML/JS with global scripts through M2, Phaser 4 as a pinned script global starting M3 (§13). *(Delivery decided at M3 S1, 2026-07-09: vendored `phaser.min.js` 4.2.1, not a CDN — §13, `DECISIONS.md`.)*
