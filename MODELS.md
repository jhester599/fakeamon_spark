...
Steps here are named **`M3S1`–`M3S10`** — the same names `PLANS/M3_OVERWORLD_PLAN.md`
§9, `ROADMAP.md`, and `roadmap.html` all use now, one row per step (the plan's
table carries the same per-step picks — if the two ever disagree, the plan
wins). **`M3S11`** (expand the wild roster — The Meadows' slice) is new work
*beyond* the plan; its pick is the roster row at the bottom of this section.
...
| 6 | Grow the wild roster, area-by-area | **Sonnet 5** | medium | Same data-entry job as M3S11, repeated per new area as gyms open |
