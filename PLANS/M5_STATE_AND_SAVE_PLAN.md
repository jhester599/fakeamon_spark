...
| **S5** ✅ | M5 Step 1 (ROADMAP) | XP & leveling | `src/progression.js` per §2; constants marked `[TUNE]`; log lines — *done 2026-08-13: `xpToNext(level) = XP_BASE × level` (10 × L) with a level cap of 30, `xpReward()` (8 × opponent level, ×0.5 for a catch, ×`MINIBOSS_XP_MULT` for M5S2), and `giveXP()` as the only way XP goes up — it returns `{levelsGained, messages}` and `battle.js` just prints them, so battles never do level maths. **`STAT_GROWTH_PER_LEVEL` switched on at the same time** (`{hp:3, attack:2, defense:1, speed:1}`) — the zeros that made `level` decorative since M2. Levelling raises current HP by exactly the max-HP gain (no free heal) and won't revive a fainted Fakeamon. Balance pass by 400-fight simulation: starter moved to level 5, The Lagoon re-levelled 10–15 → 8–12 (`DECISIONS.md` #82–84). No save-version bump — `level`/`xp` were already saved — but `parseSave` now runs `catchUpLevels` over party + box, so an M4-era save cashes in its banked XP on Continue* | ~~Sonnet 5 / medium~~ built Opus 5 / high (huddle override: this step changes every stat in the game at once) |
| **S6** ✅ | M5 Step 1 | Evolution machinery | §3: data fields, evolve-on-levelup, HP-fraction rule, `onEvolve` hook; evolved-form species entries + sprites (slicer re-run) — *done 2026-08-13: `evolvesTo`/`evolvesAt` on all 29 base species, `evolutionFor()`/`evolveNow()`/`applyEvolutions()` + the `onEvolve(individual, oldKey, newKey)` hook in `src/progression.js`, and the swap really is the planned one line (`individual.speciesKey = evolvesTo`) — everything else follows because stats/art/moves are all derived from species+level. Runs from `main.js`'s `evolveWhoeverIsReady()` after a battle and after a load, never inside battle resolution (§3). 29 evolved species sliced + credited (all were already in the verified 198, so no new licence checks). Five new "big" moves were needed: the reward for evolving is the new form's move kit, and grass had only ONE attack in the whole game* | ~~Sonnet 5 / medium~~ built Opus 5 / high (huddle override) |
