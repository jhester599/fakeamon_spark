...
**Milestone M3 — Overworld: in progress (M3S1–M3S7 done, 7 of 11).** M1 (a
full turn-based battle) is complete 🎉, and **M2 is fully done** — catching, a
team of 4 with Boxes overflow, switching fighters, **and saving**: the game
autosaves and opens on a **Continue / New Game** title screen, with **Export /
Import Save** so you can carry your adventure between browsers. **M3 brings in
Phaser** (the game engine for the map): The Meadows renders, an animated hero
walks it with the arrow keys, wild Fakeamon idle in the grass — and **walking
into one now opens the real battle** and returns you to the map (beat or catch
it and it's gone; run and it stays). **Next: M3S8** (catch-on-map depth + XP).
See `ROADMAP.md` for the full plan and `PLANS/M3_OVERWORLD_PLAN.md` for the
...
| `HOMEWORK_BACKLOG.md` | The full question bank (B1–B42, all decided) that `HOMEWORK.md` gets served from |
