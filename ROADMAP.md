...
> `config.enemyParty` is an array with an `enemyIndex`, the exact mirror of the
> player's `party[0]` (`activeOpponent()` beside `activePlayer()`). When the
> leader's Fakeamon faints, the next one is sent out and the fight carries on;
> only the last faint ends it. A wild battle passes a single `enemy` and is
> normalised to a bench of one, so **nothing about wild fights changed**. On the
> map, a third building joins the town row — bump the ⚙️ **Gym** to meet
> **Enforcer Boss**, then fight **Allagon** and the ace **AV8R** (`src/data/gyms.js`).
> Gyms allow **no running and no catching** (Jeff & Lewis's call — B1's "Run
> always works" was about *wild* battles). Winning pays `GYM_REWARD` (40 🪙) plus
> the **Gear Badge**, which shows as a chip in the HUD and pushes `theLagoon`
> onto `unlockedAreas` — so the gate M4S6 builds already has something to open.
> A rematch pays the smaller `GYM_REMATCH_REWARD` (Lewis's B17). The game's
...
| Step | What we build | ▶ You'll see |
|---|---|---|
| **1** | **Evolutions** — a Fakeamon that reaches its evolve level gets a **new sprite + higher stats**. *Nearly done (2026-08-13): **levelling works** (the M5 plan's S5 — XP buys levels, and a level is worth +3 max HP, +2 Attack, +1 Defense, +1 Speed where it changed nothing before) **and everything now evolves** (S6 — all 3 starters + all 26 wild Fakeamon, 29 evolved forms sliced and credited). Only S7, the ceremony, is left.* | **Now:** "Growler grew to level 7! ⭐" in the battle log, a Lv badge on both fighters, an XP bar on every team card — and at level 16, "What?! Growler evolved into Deviraptor! ✨" with a whole new creature on your team. **Next (S7):** the big reveal — screen flash, "What?! Growler is evolving!", sprite change on screen |
